...
((e) => {
	"function" === typeof define && define.amd
		? define(["jquery", "datatables.net"], (i) => e(i, window, document))
	...
	function x(a) {
		...
		c.on("mousedown.dtSelect", d, (b) => {
			...
			.on("mouseup.dtSelect", d, () => {
				c.css("-moz-user-select", "");
			})
			.on("click.dtSelect", d, (b) => {
		...
		e("body").on("click.dtSelect" + a.table().node().id, (c) => {
			b._select.blurable &&
	...
	e.each(
		...
		(a, c) => {
			g.ext.selector[c.type].push((b, a, f) => {
				var a = a.selected,
					e,
					g = [];
				if (!0 !== a && !1 !== a) return f;
	...
	g.ext.selector.cell.push((a, c, b) => {
		var c = c.selected,
			d,
			f = [];
		if (c === h) return b;
	...
	n("columns().select()", "column().select()", function (a) {
		...
		this.iterator("table", (b, a) => {
			j(this, "select", ["column", this[a]], !0);
		});
		return this;
	});
	n("cells().select()", "cell().select()", function (a) {
		if (!1 === a) return this.deselect();
		this.iterator("cell", (b, c, a) => {
			o(b);
			c = b.aoData[c];
	...
	n("columns().deselect()", "column().deselect()", function () {
		...
		this.iterator("table", (c, b) => {
			j(this, "deselect", ["column", this[b]], !0);
		});
		return this;
	});
	n("cells().deselect()", "cell().deselect()", function () {
		this.iterator("cell", (a, b, d) => {
			b = a.aoData[b];
	...
	e.each(["Row", "Column", "Cell"], (a, c) => {
		...
		g.ext.buttons["select" + c + "s"] = {
			...
			init: function (a) {
				a.on("selectItems.dt.DT", (a, d, e) => {
					this.active(e === b);
				});
			},
		};
	});
	e(l).on("preInit.dt.dtSelect", (a, c) => {
