...
((factory) => {
	...
	var DataTable = function (options) {
		...
		this.each(function () {
			...
			if (
				$.inArray(
					true,
					$.map(stripeClasses, (el, i) => rowOne.hasClass(el)),
				) !== -1
			) {
			...
			if (rowOne.length) {
				var a = (cell, name) =>
					cell.getAttribute("data-" + name) !== null ? name : null;
	...
	var _empty = (d) => (!d || d === true || d === "-" ? true : false);
	...
	var _numToDecimal = (num, decimalPoint) => {
	...
	var _isNumber = (d, decimalPoint, formatted) => {
	...
	var _isHtml = (d) => _empty(d) || typeof d === "string";

	var _htmlNumeric = (d, decimalPoint, formatted) => {
		if (_empty(d)) {
			return true;
		}
	...
	DataTable.util = {
		 ...
		 */
		escapeRegex: (val) => val.replace(_re_escape_regex, "\\$1"),
	};
	...
	function _fnColumnOptions(oSettings, iCol, oOptions) {
		...
		oCol.fnGetData = (rowData, type, meta) => {
		...
		oCol.fnSetData = (rowData, val, meta) =>
			_fnSetObjectDataFn(mDataSrc)(rowData, val, meta);
	...
	function _fnGetObjectDataFn(mSource) {
		if ($.isPlainObject(mSource)) {
			...
			return (data, type, row, meta) => mSource(data, type, row, meta);
		} else if (
	...
	function _fnAjaxUpdate(settings) {
		if (settings.bAjaxDataGet) {
			...
			_fnBuildAjax(settings, _fnAjaxParameters(settings), (json) => {
	...
	function _fnAjaxUpdateDraw(settings, json) {
		...
		// Support both
		var compat = (old, modern) =>
			json[old] !== undefined ? json[old] : json[modern];
	...
	function _fnExtend(out, extender, breakRefs) {
		...
		for (var prop in extender) {
			if (Object.hasOwn(extender, prop)) {
				val = extender[prop];
	...
	function _fnCallbackFire(settings, callbackArr, eventName, args) {
		...
		if (callbackArr) {
			ret = $.map(settings[callbackArr].slice().reverse(), (val, i) =>
				val.fn.apply(settings.oInstance, args),
			);
		}
	...
	var _selector_run = (type, selector, selectFn, settings, opts) => {
		var out = [],
			res,
			a,
			i,
	...
	var _selector_row_indexes = (settings, opts) => {
		...
		if (_fnDataSource(settings) == "ssp") {
			...
			if (search == "none") {
				...
				a = $.map(displayMaster, (el) =>
					!Object.hasOwn(displayFilteredMap, el) ? el : null,
				);
			}
	 ...
	 *
	 */
	var __row_selector = (settings, selector, opts) => {
		var rows;
		var run = (sel) => {
	...
	_api_register("rows()", function (selector, opts) {
		...
		var inst = this.iterator(
			"table",
			(settings) => __row_selector(settings, selector, opts),
			1,
		);
	...
	_api_register("rows().nodes()", function () {
		return this.iterator(
			"row",
			(settings, row) => settings.aoData[row].nTr || undefined,
			1,
		);
	});
	...
	_api_register("rows().data()", function () {
		return this.iterator(
			true,
			"rows",
			(settings, rows) => _pluck_order(settings.aoData, rows, "_aData"),
			1,
		);
	});
	...
	_api_registerPlural("rows().remove()", "row().remove()", function () {
		this.iterator("row", (settings, row, thatIdx) => {
			...
			_fnDeleteIndex(this[thatIdx], row, false); // maintain local indexes
	...
	var __details_display = (api, show) => {
		var ctx = api.context;
	...
	var __columnData = (settings, column, r1, r2, rows) => {
		var a = [];
	...
	var __column_selector = (settings, selector, opts) => {
		...
		var run = (s) => {
			...
			if (typeof s === "function") {
				...
				return $.map(columns, (col, idx) =>
					s(idx, __columnData(settings, idx, 0, 0, rows), nodes[idx])
						? idx
						: null,
				);
			}
				...
				switch (match[2]) {
					...
					case "visible": {
						...
						if (idx < 0) {
							...
							var visColumns = $.map(columns, (col, i) =>
								col.bVisible ? i : null,
							);
	...
	_api_registerPlural(
		...
		function (selector, opts) {
			return this.iterator(
				"column",
				(settings, column) => settings.aoColumns[column].nTh,
				1,
			);
		},
	);
	...
	_api_registerPlural(
		...
		function (selector, opts) {
			return this.iterator(
				"column",
				(settings, column) => settings.aoColumns[column].nTf,
				1,
			);
		},
	);
	...
	_api_registerPlural("columns().dataSrc()", "column().dataSrc()", function () {
		return this.iterator(
			"column",
			(settings, column) => settings.aoColumns[column].mData,
			1,
		);
	});
	...
	_api_registerPlural("columns().nodes()", "column().nodes()", function () {
		return this.iterator(
			"column-rows",
			(settings, column, i, j, rows) =>
				_pluck_order(settings.aoData, rows, "anCells", column),
			1,
		);
	});
	...
	_api_registerPlural(
		...
		"column().visible()",
		function (vis, calc) {
			var ret = this.iterator("column", (settings, column) => {
				if (vis === undefined) {
			...
			if (vis !== undefined) {
				...
				this.iterator("column", (settings, column) => {
	...
	_api_register("cells()", function (rowSelector, columnSelector, opts) {
		...
		if (columnSelector === null || columnSelector === undefined) {
			return this.iterator("table", (settings) =>
				__cell_selector(settings, rowSelector, _selector_opts(opts)),
			);
		}
	...
	_api_register("cells().data()", function () {
		return this.iterator(
			"cell",
			(settings, row, column) => _fnGetCellData(settings, row, column),
			1,
		);
	});
	...
	_api_registerPlural("cells().cache()", "cell().cache()", function (type) {
		...
		return this.iterator(
			"cell",
			(settings, row, column) => settings.aoData[row][type][column],
			1,
		);
	});
	...
	_api_registerPlural("cells().render()", "cell().render()", function (type) {
		return this.iterator(
			"cell",
			(settings, row, column) => _fnGetCellData(settings, row, column, type),
			1,
		);
	});
	 ...
	 *    } );
	 */
	DataTable.tables = DataTable.fnTables = (visible) => {
		var api = false;
		...
		var a = $.map(DataTable.settings, (o) => {
	...
	$.extend(extPagination, {
		simple: (page, pages) => ["previous", "next"],

		full: (page, pages) => ["first", "previous", "next", "last"],

		numbers: (page, pages) => [_numbers(page, pages)],
	...
	$.extend(DataTable.ext.type.detect, [
		...
		(d, settings) =>
			_empty(d) || (typeof d === "string" && d.indexOf("<") !== -1)
				? "html"
				: null,
	]);
	...
	var __numericReplace = (d, decimalPlace, re1, re2) => {
		if (d !== 0 && (!d || d === "-")) {
			return -Infinity;
		}
	...
	function _addNumericSort(decimalPlace) {
		$.each(
			{
				// Plain numbers
				num: (d) => __numericReplace(d, decimalPlace),

				// Formatted numbers
				"num-fmt": (d) =>
					__numericReplace(d, decimalPlace, _re_formatted_numeric),

				// HTML numeric
				"html-num": (d) => __numericReplace(d, decimalPlace, _re_html),

				// HTML numeric, formatted
				"html-num-fmt": (d) =>
					__numericReplace(d, decimalPlace, _re_html, _re_formatted_numeric),
			},
			(key, fn) => {
